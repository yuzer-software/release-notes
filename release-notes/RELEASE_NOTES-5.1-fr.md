...
Lors du changement de contact d'un groupe de facturation, si le nouveau contact possède un programme de fidélité applicable automatiquement, alors celui-ci est bien ajouté au groupe de facturation. Si un programme de fidélité était déjà en place, vous avez la possibilité de le supprimer.
...
Le groupe de facturation dispose désormais d'un statut d'exonération de TVA permettant d'identifier explicitement son état.  
...
Un message d'avertissement vous informe si une ligne ne semble pas correspondre à ce statut.
...
Le résumé de la vente d'un panier de produit identifié avec reprises inclut une ligne explicite de _financement des reprises_ (somme des Soldes restant dûs aux organismes de financement).
...
En vue de la préparation de FactureX, nous avons mis à jour notre service de génération de PDF.  
Par ailleurs, nous avons apporté une amélioration générale de la mise en page (alignements, etc) et nous avons également ajouté le libellé sur les moyens de paiement pour les documents de facture.  
À noter que la police du texte a légèrement changé.

## Annulation de factures d'acompte

Il est maintenant possible d'annuler les factures d'acompte. L'annulation d'une facture d'acompte crée un document de facture d'avoir sur acompte.

![deposit-cancellation-form](https://raw.githubusercontent.com/yuzer-software/release-notes/master/release-notes/5.1/deposit-cancellation-form.webp?w=450px)

A noter que vous devez saisir un montant de remboursement égale à celui de la facture d'acompte correspondant.

## Export de documents comptables

Vous pouviez paramétrer le nom du fichier en fonction de variables horaires, comme le numéro de la semaine. La date utilisée pour formater le nom du fichier était le jour de l'export, c'est-à-dire le jour _suivant_ la période exportée. Si vous exportiez une semaine donnée, le nom du fichier avait donc le n° de la semaine _suivant_ la semaine exportée. Vous pouvez désormais choisir d'utiliser la date du début de la période exportée.
...
- Nous avons revu l'option de l'envoie d'email sur les formulaires de pré-édition de documents pour apporter plus de consistance entre l'édition des différents documents. L'option est maintenant toujours pré-cochée en fonction de l'email configuré par défaut sur la fiche cliente.
...
- Correction de l'affichage du contact acheteur qui n'était pas toujours rafraîchit après un changement.
...
- Affichage d'un message d'erreur dans le cas d'un échec de synchronisation lors de l'édition d'un document de reçu.
- Les documents de panier n'étaient pas tous affichés dans le menu latéral après l'édition d'une annulation. C'est maintenant corrigé.
